    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                $query = mysqli_query($conn, "INSERT INTO tb_daftar_menu (foto,nama_menu,keterangan,kategori,harga,stok) VALUES ('" . $kode_rand . "_" . $_FILES['foto']['name'] . "','$nama_menu','$keterangan','$kat_menu','$harga','$stok')");
                if ($query) {
                    $message = '<script>alert("Data menu berhasil ditambahkan!"); window.location = "../menu";</script>';
                } else {
                    $message = '<script>alert("Data menu gagal ditambahkan!"); window.location = "../menu";</script>';
                }
            } else {
